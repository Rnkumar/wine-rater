...
const AddWineForm = (props) =>{
  ...
  const handleCheckBoxChange = (event) => {
    ...
    if (stateData[event.target.name] instanceof Array) {
      if (stateData[event.target.name].includes(event.target.value)) {
        stateData[event.target.name] = stateData[event.target.name].filter(
          value => value !== event.target.value
        );
      } else {
        stateData[event.target.name].push(event.target.value);
      }
    }
  }
